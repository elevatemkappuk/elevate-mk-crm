...
@Component({
  ...
  template: `
    <fieldset>
      <legend>{{ label() }}</legend>
      <label class="search-label" [for]="searchId">Search {{ label().toLowerCase() }}</label>
      ...
      <small aria-live="polite">{{ matching().length }} options</small>
    </fieldset>
  `,
  styles: `
    ...
    .search-label,small { font-size:var(--crm-font-sm); color:var(--crm-text-secondary); }
